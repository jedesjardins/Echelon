...
if(item != noone)
{
	if(state_attack and state_attack_hand == "right")
	{
		...
		//draw_sprite_ext(item[? "state_sprite"], image_index, x+off_x[image_index], y+off_y[image_index], 1, 1, gear_angle[? state_direction]+45-(45*image_index), c_white, 1);
	...
	else if (!state_attack)
	{
		...
		draw_sprite_ext(item[? "state_sprite"], 2, x+off_x[image_index], y+off_y[image_index], 1, 1, gear_angle[? state_direction], c_white, 1);
	}
}